#################################
#Enriched clusters (CD_C) in HMP2-IBD
#HMP2-IBD profiler
#################################
library(tidyverse)
library(ggsci)
...
#########################
#download data 
quantifier_results_DNA <- read_delim("HMP2-IBD/combined-HMP2-IBD-metagenomes-quantifier-results.txt", col_names = F, delim = "\t")
names(quantifier_results_DNA) <- c("bgcName", "Coverage", "RPKM", "Sample") # add column names

quantifier_results_RNA <- read_delim("HMP2-IBD/combined-HMP2-IBD-metatranscriptomes-quantifier-results.txt", col_names = F, delim = "\t")
names(quantifier_results_RNA) <- c("bgcName", "Coverage", "RPKM", "Sample") # add column names

enriched_hmp2_cd <- read_csv("HMP2-IBD/supplementary-tables/lefse_CD_C_supp_table_1.csv",col_names = T) %>% filter(class_discriminitive == "Crohn's Disease" )
################################################################################
# HMP2 IBD metadata 
...
quantifier_df_matched_enriched <- quantifier_df_matched %>% filter(bgcName %in% enriched_hmp2_cd$BGC)
#plot RNA and DNA by BGC 
quantifier_df_matched_enriched %>% ggplot(., aes(Sample, Coverage,fill=data_type))+  geom_bar(stat = "identity",position = "dodge")+facet_wrap(~bgcName)+ theme(axis.text.x = element_text(angle = 90, hjust = .5, size =6)) + geom_hline(aes(yintercept=10))
...
#79 rows for DNA is 0s 
#6 rows for RNA is 0s 
...
enriched_match_dna_rna %>% ggplot(., aes(ParticipantID, logRatio, fill=GroupAttribute))+  geom_col()+facet_wrap(~bgcName)+ theme(plot.title = element_text(hjust = 0.5,face = "bold",size = "24"))+
  theme(axis.text.x = element_text(angle = 90, hjust = .5)) + scale_fill_npg()
