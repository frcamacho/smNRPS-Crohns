...
# gplots::venn(list(enrichedCD_C = enrichedUC_C_list$BGC, enrichedUC_CD = enrichedUC_CD_list$BGC))
```
...
```{r}
#Heatmap of RPKM total 
# 
# lefse_CD_C_recode_important_feat<-lefse_CD_C_supp_table %>% filter(class_discriminitive!= "-" & LDA_score!="-") 
# heatmap_RPKM <- filter_quantifier_df_updated  %>% filter(GroupAttribute == "CD" | GroupAttribute == "C") %>% filter(bgcName %in% lefse_CD_C_recode_important_feat$BGC)%>% select(-c(4))
# data_wide <- dcast(heatmap_RPKM, bgcName ~ Sample, value.var="RPKM") 
...
# 
# annotations<-read.csv("sample_metadata.csv", header=TRUE, sep=",")
# rownames(annotations)<-annotations$Sample
# annotations_col<-annotations[-c(1)]
# 
# #Colors for heatmap 
# bk = unique(c(seq(0,0, length=100), seq(0.06,90,length=1000)))
# hmcols<- colorRampPalette(c("white","blue","green","yellow","orange","red"))(length(bk)-1)
# 
# pheatmap::pheatmap(data_wide, scale = "none", breaks=bk, color=hmcols,annotation= annotations_col,show_rownames=FALSE, show_colnames = FALSE,
#                    clustering_distance_rows = "correlation", clustering_distance_cols = "correlation", 
#                    clustering_method = "complete",main="Complete clustering using correlation distance metric of RPKM of BGCs")
# 
# 
# km<-kmeans(data_wide,2)
# m.kmeans<- cbind(data_wide, km$cluster) 
# order.m.kmeans<- order(m.kmeans[,93])
# m.kmeans<- m.kmeans[order.m.kmeans,] # order the matrix according to the order of the last column
# pheatmap::pheatmap(m.kmeans[,1:92], scale = "none", breaks=bk, color=hmcols,annotation= annotations_col,show_rownames=FALSE, show_colnames = FALSE, main="Kmean clustering (2) of Relative Score of BGCs above 50% breadth")
