...
species_df<- read_delim("/Users/francinecamacho/Documents/Donia_analysis/smNRPS-manuscript/smNRPS-Crohns/data/HMP2-IBD/refseq_against_HMP2-IBD-SPAdes_uniqueBGCs_tabular-file-tophit_500-hits-bitscore",
                        delim = "\t", col_names = F) 
names(species_df)<-c( "sseqid","stitle", "sacc", "qseqid", "qlen", "qcovs", "pident", "evalue", "bitscore", "qstart", "qend")

cluster_ids <- read_lines("HMP2-IBD-metagenomes_uniqueBGCs-list.txt")
data_df<- species_profiler(species_df, 50, cluster_ids)
write_delim(data_df, "species_results-HMP2-IBD.txt", delim="\t", col_names = T)